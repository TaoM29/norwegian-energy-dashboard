...
# pages/11_Energy_Consumption.py
import pandas as pd
...
GROUP_COLORS = {
    ...
    "secondary": "#E15759", "tertiary": "#B07AA1", "industry": "#F28E2B",
...
def totals_df(area: str, year_: int, groups: tuple[str, ...]) -> pd.DataFrame:
    ...
    if groups:
        match["consumption_group"] = {"$in": list(groups)}
    pipe = [
        {"$match": match},
        {"$group": {"_id": "$consumption_group", "total_kwh": {"$sum": "$quantity_kwh"}}},
        {"$project": {"_id": 0, "consumption_group": "$_id", "total_kwh": 1}},
        {"$sort": {"total_kwh": -1}},
    ]
...
def hourly_df(area: str, year_: int, month_: int, groups: tuple[str, ...]) -> pd.DataFrame:
    ...
    if groups:
        match["consumption_group"] = {"$in": list(groups)}
    rows = list(cons.find(match, {"_id": 0, "consumption_group": 1, "start_time": 1, "quantity_kwh": 1}))
    if not rows:
...
if df_tot.empty:
    ...
    fig = px.pie(
        df_tot, names="consumption_group", values="total_kwh",
...
if df_hour.empty:
    ...
    pivot = (
        df_hour.pivot_table(index="start_time", columns="consumption_group",
