...
export function DemandProfilesView({ result }: { result: DemandProfilesResponse }) {
  ...
  return <section className="analysis-panel profile-workspace" aria-label="Daily household demand profiles">
    <header className="profile-heading">
      <div><div className="analysis-title"><h2>Daily demand profiles</h2>
        <HelpTip label="Daily profile bands" iconOnly>Median hourly demand in Oslo time; shading shows the 10th–90th percentile across days, not a confidence interval.</HelpTip></div></div>
    ...
    <div className="profile-coverage"><span><strong>{number(result.coverage.touchedDays - result.coverage.profileEligibleDays, 0)}</strong> local dates excluded</span>{result.coverage.zeroMeanDays > 0 && <span><strong>{number(result.coverage.zeroMeanDays, 0)}</strong> zero-mean days excluded from shape only</span>}<HelpPanel label="Coverage & method">
      ...
      <p>Source: {result.dataMode === "fixture" ? "synthetic fixture" : "published Elhub household consumption"}.</p>
    </HelpPanel></div>
    <details className="profile-representatives"><summary>Representative observed days</summary><div>{groups.map((group) => <div key={group.group}><span>{groupNames[group.group] || group.group}</span>{group.representative ? <button type="button" onClick={() => inspectDate(group.representative!.date)} aria-label={`Inspect ${groupNames[group.group] || group.group} representative day ${group.representative.date}`}>{group.representative.date} <span aria-hidden="true">↗</span></button> : <span>Unavailable</span>}</div>)}</div></details>
