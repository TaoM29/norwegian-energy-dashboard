...
test("feature ablation shows fixed-cohort variants, signed contrasts and missing ranges", async ({ page }) => {
  ...
  await expect(panel.getByRole("row", { name: /Calendar \+ demand \+ eligible weather/ })).toContainText("12.0");
  const contrasts = page.getByRole("region", { name: "Feature ablation added-input contrasts" });
  await expect(contrasts.getByRole("row", { name: /Add demand history/ })).toContainText("-4.0 kWh");
  await expect(contrasts.getByRole("row", { name: /Add demand history/ })).toContainText("+2.0 pp");
  await expect(contrasts.getByRole("row", { name: /Add demand history/ })).toContainText("8 dates: Unavailable (Too few date blocks)");
  await expect(contrasts.getByRole("row", { name: /Add eligible weather/ })).toContainText("+1.0 kWh");
  ...
  await expect(page.getByRole("region", { name: "Added-input pinball changes" })).toContainText("lower -1.0 kWh");
  const uncertainty = page.getByRole("region", { name: "Add demand history uncertainty ranges" });
  await expect(uncertainty.getByRole("row", { name: /2 dates/ })).toContainText("-3.0 pp to +6.0 pp");
