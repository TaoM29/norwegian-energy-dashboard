...
export default function SensitivityView({ area, onAreaChange }: { area: string; onAreaChange: (area: string) => void }) {
  ...
  return <div className="sensitivity-view">
    <div className="sensitivity-intro">
      ...
      <label>Price area
        <Select priceArea aria-label="Sensitivity price area" value={area} onChange={(event) => onAreaChange(event.target.value)}>
      ...
      {data ? <>
        ...
        <section className="analysis-panel">
          ...
          <details><summary>Residual values and sample counts</summary>
            <div className="sensitivity-two-col">
              <div className="sensitivity-table-wrap"><table><thead><tr><th>Lag (h)</th><th>Correlation</th><th>Pairs</th></tr></thead><tbody>{data.residuals.acf.map((row) => <tr key={row.lagHours}><td>{row.lagHours}</td><td>{formatDisplayValue(row.correlation, 3)}</td><td>{number(row.pairs, 0)}</td></tr>)}</tbody></table></div>
