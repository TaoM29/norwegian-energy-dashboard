# app2.py — IND320 Part 2 RESUB (single-file Streamlit app)
...
def load_weather_csv(path: Path) -> pd.DataFrame:
    ...
    # Standardiser kolonnenavn fra Part 1
    rename_map = {
...
def load_elhub_csv(path: Path) -> pd.DataFrame:
    ...
    df = df.rename(columns=str.strip)
    # Gjør robust tz-håndtering: konverter til UTC-aware -> dropp tz (naiv)
